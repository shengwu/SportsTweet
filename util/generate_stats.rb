...
# List top players and store result
begin
...
# List top teams and store result
begin
  item = CachedResult.where(:name => "top_teams")[0]
  item.update_attributes(:result => show_popular_teams())
rescue Exception => e
  CachedResult.create(:name => 'top_teams',:result => show_popular_teams())
end
